...
$app->post("/tigo/callback",function(Request $request,  Response $response, $args = []){
    ...
    $paymentResponse = $furahitechPay->mobilePaymentCallback($transactionRef,$transactionStatus);
    return $response->withJson($paymentResponse,200);
});
...
$app->post("/vodacom/callback",function(Request $request,  Response $response, $args = []){
    ...
    $furahitechPay = new FurahitechPay();
    $transactionRef = $paymentDetails->uid;
    $transactionStatus = $paymentDetails->code == 200 ? $paymentDetails->code: 404;
